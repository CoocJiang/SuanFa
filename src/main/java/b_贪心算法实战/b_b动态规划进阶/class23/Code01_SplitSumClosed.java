...
public class Code01_SplitSumClosed {
	...
	public static int right3(int [] arr){
		if (arr == null || arr.length < 2) {
			return 0;
		}
		int sum = 0;
		...
		dp[0][0] = arr[0]>border ? 0:arr[0];
		for (int i=1;i< dp.length;i++){
			int p1= dp[i-1][0]+arr[i] > border ? dp[i-1][0]:dp[i-1][0]+arr[i];
			int p2 = dp[i-1][1]+arr[i] > border ? dp[i-1][1]:dp[i-1][1]+arr[i];
